...
";
  ...
  // save the scss file
  $stylefile = \Drupal::service('file_system')->saveData($styleData, $styleFile, FileSystemInterface::EXISTS_REPLACE);
  // check for errors
  if ($stylefile) {
    \Drupal::messenger()->addMessage(t('SCSS file saved: <strong>'. str_replace(realpath(".") . base_path(), "", $styleFile) .'</strong>'));
  }
  else {
    \Drupal::messenger()->addMessage(t('WTF004: SCSS save error... : function _omega_update_style_scss()'), 'error');
  }
    ...
    if (!preg_match($ignore, $file)) {
      ...
      if (is_dir($source . '/' . $file)) {
        ...
        if (pathinfo($file, PATHINFO_EXTENSION) == $filetype) {
          ...
          // check for errors
          if ($cssfile) {
            \Drupal::messenger()->addMessage(t('CSS file saved: <strong>'.str_replace(realpath(".") . base_path(), "", $cssfile).'</strong>'));
          }
          else {
            \Drupal::messenger()->addMessage(t('WTF005: CSS save error... : function scssDirectoryScan()'), 'error');
          }
        }
      }
    }
...
function _omega_save_database_layout($layout, $layout_id, $theme, $generate = FALSE) {
  ...
  if ($layoutConfig->getOriginal() == $layout) {
    ...
    // check for errors
    if ($saved) {
      \Drupal::messenger()->addMessage(t('Layout <em>' . $layout_id . '</em> updated: <strong>'.$theme . '.layout.' . $layout_id.'</strong>'));
    }
    else {
      \Drupal::messenger()->addMessage(t('WTF002: Layout configuration error... : function _omega_save_database_layout()'), 'error');
    }
  }
  ...
  if ($generate) {
    if ($layoutConfigGenerated->getOriginal() != $layout) {
      ...
      if ($saved) {
        \Drupal::messenger()->addMessage(t('Layout <em>' . $layout_id . '</em> updated: <strong>'.$theme . '.layout.' . $layout_id.'.generated</strong>'));
      }
      else {
        \Drupal::messenger()->addMessage(t('WTF003: Layout configuration error... : function _omega_save_database_layout()'), 'error');
      }
      return true;
    }
    else {
...
function _omega_save_layout_files($scss, $theme, $layout_id) {
  ...
  // save the scss file
  $scssfile = \Drupal::service('file_system')->saveData($scss, $layoutscss, FileSystemInterface::EXISTS_REPLACE);
  // check for errors
  if ($scssfile) {
    \Drupal::messenger()->addMessage(t('SCSS file saved: <strong>'. str_replace(realpath(".") . base_path(), "", $scssfile) .'</strong>'));
  }
  else {
    \Drupal::messenger()->addMessage(t('WTF001: SCSS save error... : function _omega_save_layout_files()'), 'error');
  }
  ...
  if ($compile) {
    ...
    // save the css file
    $cssfile = \Drupal::service('file_system')->saveData($css, $layoutcss, FileSystemInterface::EXISTS_REPLACE);
    // check for errors
    if ($cssfile) {
      \Drupal::messenger()->addMessage(t('CSS file saved: <strong>'.str_replace(realpath(".") . base_path(), "", $cssfile).'</strong>'));
    }
    else {
      \Drupal::messenger()->addMessage(t('WTF002: CSS save error... : function _omega_save_layout_files()'), 'error');
    }
  }
  ...
  elseif (theme_get_setting('show_compile_warning', $theme)) {
    \Drupal::messenger()->addMessage(t("Since <strong>Compile SCSS Directly</strong> is disabled, please ensure Compass or an alternative SCSS compiler is set to watch for these saved changes. You can disable this warning under <strong>Default Options</strong>."), "warning");
  }
