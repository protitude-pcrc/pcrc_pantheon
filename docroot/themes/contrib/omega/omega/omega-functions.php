...
function _omega_getAvailableBreakpoints($theme) {
  ...
  if ($breakpoints_module == TRUE) {
    ...
    \Drupal::messenger()->addMessage(t('Omega requires the <b>Breakpoint module</b>. Open the <a href="@extendpage" target="_blank">Extend</a> page and enable Breakpoint.', array('@extendpage' => base_path() . 'admin/modules')), 'warning');
  }
...
function _omega_getActiveBreakpoints($layout, $theme) {
  ...
  if ($breakpointGroup) {
    ...
    \Drupal::messenger()->addMessage('The breakpoint group for your theme could not be found. Using default Omega version instead.', 'warning');
