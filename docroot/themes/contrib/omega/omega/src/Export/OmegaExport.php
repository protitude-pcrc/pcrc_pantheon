...
class OmegaExport implements OmegaExportInterface {
  ...
  public function saveExport( FormStateInterface $form_state) {
    ...
    switch ($this->build['type']) {
      case "clone":
        ...
        if (!$infoUpdated) {
          \Drupal::messenger()->addMessage("Could not save " . $this->build['machine'] . ".info.yml", "error");
        }
  ...
  public function fileCopy($source, $destination) {
    if (file_exists($source)) {
      ...
      if ($copy) {
        ...
        // @debug
        \Drupal::messenger()->addMessage(t('File copy failed on function: <strong><small>copy(' . $source . ', ' . $destination . ');</small></strong>'), 'error');
        return false;
      }
    }
    // @debug
    \Drupal::messenger()->addMessage(t('Source file not found: <strong><small>' . $source . '</small></strong>'), 'error');
    return false;
  }
  ...
  protected function generateThemeFile() {
    ...
    if ($themeFile) {
      ...
      \Drupal::messenger()->addMessage("Could not save " . $this->build['machine'] . ".theme file", "error");
    }
  ...
  protected function generateConfigrb() {
    ...
    if ($this->build['theme_configrb_create']) {
      ...
      if ($configRbFile) {
        ...
        \Drupal::messenger()->addMessage("Error saving config.rb.", "error");
      }
    }
    else {
      $this->fileRemove($destination);
  ...
  protected function generateGemfile() {
    ...
    if ($this->build['theme_gemfile_create']) {
      ...
      if (!$Gemfile) {
        \Drupal::messenger()->addMessage("Error saving Gemfile.", "error");
      }
    }
    else {
